...
date: '14/01/2022 (v. `r format(Sys.Date(), "%d/%m/%Y")`)'
lang: 'pt-br'
output: 
...
* Cada item que pede para você construir um intervalo de confiança ou para você fazer um teste de hipóteses [deve ser resolvido de duas maneiras]{.hl} (exceto quando houver instrução dizendo o contrário):
  ...
  1. Fazendo os cálculos passo a passo e
...
1. Uma amostra aleatória de $86$ pessoas inclui $41$ fumantes.
   ...
   a. Faça um teste de hipóteses para avaliar a afirmação de que o percentual de fumantes na população é de $50\%$ (em oposição a diferente de $50\%$). Use $\alpha = 0{,}05$.
   
      ```{r child='fumantes2.Rmd', eval=gabarito}
      
      ```

1. Outra amostra aleatória, de $30$ pessoas, tem $30$ fumantes. 

   a. Construa um intervalo de confiança de $95\%$ para o percentual de fumantes na população de onde foi coletada a amostra. Comente o resultado.

   a. Uma maneira de evitar este problema é [adicionar $4$ elementos artificiais à amostra: $2$ fumantes e $2$ não-fumantes.]{.hl}^[Se estiver interessado, leia [este artigo](https://scholar.google.com/scholar?q=Approximate%20Is%20Better%20than%20%E2%80%9CExact%E2%80%9D%20for%20Interval%20Estimation%20of%20Binomial%20Proportions).] Refaça o item anterior com esta nova amostra.
...
1. Em uma amostra de $400$ alunos universitários, $150$ completaram a graduação em $4$ anos ou menos. Teste a afirmação de que, na população, $40\%$ dos alunos universitários completam a graduação neste tempo.


# Estimativa do tamanho de uma multidão { #multidao }

Observe a foto abaixo,^[[Link para a fonte.](https://web.archive.org/web/20101008192855/http://www.lifeslittlemysteries.com/how-is-crowd-size-estimated--1074/)] que mostra [parte]{.hl} de uma multidão. A quantidade de pessoas em cada setor quadrado foi estimada por um algoritmo.
...
1. Faça um teste de hipóteses para avaliar a afirmação de que a multidão [inteira]{.hl} consiste de [$35$ mil pessoas ou mais]{.hl}. Use $\alpha = 0{,}05$.
...
1. Teste a afirmação de que a pulsação média de mulheres adultas é menor do que $70$ bpm. Use $\alpha = 0{,}05$. Use o fato de que $\sigma = 12{,}54$ bpm.
...
# Para quem gosta de programar: *bootstrapping* (1)
...
# Para quem gosta de programar: *bootstrapping* (2)
...
# Sonhando em cores ou em preto e branco
...
# Para quem gosta de programar: *bootstrapping* (3)
